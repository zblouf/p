...
# Copyright (c) 2006-2008, Romain Gaches, <romain@ssji.net>
...
class DbGeneric:
    ...
    def run_query_with_result(self, sql_query):
        self._cursor = self._connection.cursor()
        self._cursor.execute(sql_query)
        return self._cursor

    def run_query_commit_with_result(self, sql_query):
        self._cursor = self._connection.cursor()
        self._cursor.execute(sql_query)
        self.commit()
        return self._cursor
    ...
    def run_query_commit(self, sql_query):
        self._cursor = self._connection.cursor()
        self._cursor.execute(sql_query)
        self.commit()
        return self._cursor
    ...
    def query_dict(self, sql_query):
        result = self.run_query_with_result(sql_query)
        if result:
